...
module.exports = class Issues extends Plugin {
  ...
  assign(context, ...assignees) {
    return context.github.issues.addAssigneesissue(context.issue({assignees}));
  }
  ...
  unlabel(context, ...labels) {
    return labels.map(label => context.github.issues.removeLabel(context.issue({name: label})));
  }
